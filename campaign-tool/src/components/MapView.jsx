...
/**
 * Compute bounding-box center from an array of SVG path strings/objects.
 * Works with paths generated by coordinatesToSvgPath (M x y L x y ... Z).
 */
const computePathsCenter = (pathItems) => {
  let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
  let found = false;

  pathItems.forEach(item => {
    const pathStr = typeof item === 'string' ? item : (item.svgPath || '');
    const matches = pathStr.matchAll(/[ML]\s*(-?\d+\.?\d*)[,\s]+(-?\d+\.?\d*)/gi);
...
const MapView = ({ territories, selectedTerritory, onTerritoryClick, onTerritoryDoubleClick, isCountyView = false, pendingBattleTerritoryIds = [] }) => {
  ...
  // Resolve a center point for every territory (state-based, legacy, county-based)
  const territoryCenters = useMemo(() => {
    const centers = {};
    territories.forEach(t => {
      if (t.center) { centers[t.id] = t.center; return; }
      if (t.labelPosition) { centers[t.id] = t.labelPosition; return; }
      if (countyPaths[t.id]) {
        const c = computePathsCenter(countyPaths[t.id]);
        if (c) { centers[t.id] = c; return; }
      }
      if (t.countyPaths?.length > 0) {
        const c = computePathsCenter(t.countyPaths);
        if (c) { centers[t.id] = c; return; }
      }
      if (t.svgPath || t.coordinates?.data) {
        const c = computePathsCenter([t.svgPath || t.coordinates.data]);
        if (c) { centers[t.id] = c; }
      }
    });
    return centers;
  }, [territories, countyPaths]);

  // Compute contested edges: unique pairs of adjacent territories with different owners
  const contestedEdges = useMemo(() => {
    const edges = [];
    const seen = new Set();

    territories.forEach(t => {
      if (!t.adjacentTerritories) return;
      const from = territoryCenters[t.id];
      if (!from) return;

      t.adjacentTerritories.forEach(adjId => {
        const key = [t.id, adjId].sort().join('|');
        if (seen.has(key)) return;
        seen.add(key);

        const adj = territories.find(a => a.id === adjId);
        if (!adj || t.owner === adj.owner) return;

        const to = territoryCenters[adjId];
        if (!to) return;

        edges.push({ from: t, to: adj, fromCenter: from, toCenter: to });
      });
    });

    return edges;
  }, [territories, territoryCenters]);
    ...
    <div className="bg-slate-800 rounded-lg border border-slate-700 p-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-4 text-sm">
          ...
          {contestedEdges.length > 0 && (
            <div className="flex items-center gap-2">
              <div className="w-4 h-0 border-t-2 border-dashed border-red-500 opacity-60"></div>
              <span className="text-slate-300">Attackable</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="relative bg-slate-900 rounded-lg p-4">
        ...
        >
          {/* Arrow marker for contested borders */}
          <defs>
            <marker
              id="attack-arrow"
              viewBox="0 0 10 7"
            ...
            >
              <path d="M 0 0.5 L 10 3.5 L 0 6.5 L 2 3.5 Z" fill="#dc2626" />
            </marker>
          </defs>
          <g transform={`translate(${panX}, ${panY}) scale(${zoom})`}>
            ...
            {/* Contested border arrows */}
            {contestedEdges.map((edge, i) => {
              const dx = edge.toCenter.x - edge.fromCenter.x;
              const dy = edge.toCenter.y - edge.fromCenter.y;
              // Shorten to middle 40% so arrows don't overlap labels
              const x1 = edge.fromCenter.x + dx * 0.3;
              const y1 = edge.fromCenter.y + dy * 0.3;
              const x2 = edge.fromCenter.x + dx * 0.7;
              const y2 = edge.fromCenter.y + dy * 0.7;
              ...
              return (
                <line
                  ...
                  x2={x2} y2={y2}
                  stroke="#dc2626"
                  strokeWidth={hasCountyData ? '1' : '1.5'}
                  strokeDasharray="6 3"
                  opacity="0.35"
