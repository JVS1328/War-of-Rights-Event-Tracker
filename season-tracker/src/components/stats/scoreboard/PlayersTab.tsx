...
export function PlayersTab({
  ...
  const { usa, csa } = useMemo(() => {
    ...
    for (const p of sb.players) {
      if (searchTrimmed && !p.name.toLowerCase().includes(searchTrimmed)) continue;
      if (p.team === 'USA') u.push(p);
      else c.push(p);
    }
    return { usa: u, csa: c };
  }, [sb.players, searchTrimmed]);

  // When searching, flatten the unit grouping — the user is hunting one name.
  const showUnitGroups = sortBy === 'unit' && !searchTrimmed;
  const allEmpty = usa.length === 0 && csa.length === 0;

  const totals = (rows: ScoreboardPlayer[]) =>
    rows.reduce((acc, p) => ({ kills: acc.kills + p.kills, deaths: acc.deaths + p.deaths }), {
      kills: 0,
      deaths: 0,
    });
...
function TeamBlock({
  ...
  totals: { kills: number; deaths: number };
  ...
  return (
    <div className="mb-4">
      ...
      {showUnitGroups ? (
        ...
        <PlayerCardList
          rows={[...rows].sort((a, b) => comparePlayers(a, b, sortBy))}
          isOfficer={isOfficer}
          lookup={lookup}
          killStance={killStance}
          onOpenPlayer={onOpenPlayer}
...
function RegimentGroup({
  ...
  return (
    <div className="border-t border-[color:var(--color-border)]">
      <div className="px-3 py-1 bg-[color:var(--color-bg-1)] text-[10px] uppercase tracking-wider font-mono flex justify-between items-center gap-3 flex-wrap">
        <span className="text-[color:var(--color-text-0)]">{regiment ?? 'Untagged'}</span>
      ...
      <PlayerCardList
        rows={players.slice().sort((a, b) => a.name.localeCompare(b.name))}
        isOfficer={isOfficer}
        lookup={lookup}
        killStance={killStance}
        onOpenPlayer={onOpenPlayer}
