...
/**
 * Base CP cost for attacking a territory (before VP multiplier)
 */
export const BASE_ATTACK_COST = 75;
...
/**
 * Calculate CP loss for attacker based on casualties
 ...
 */
export function calculateAttackerCPLoss(pointValue, casualties, totalCasualties) {
  // Validate inputs
  if (![5, 10, 15].includes(pointValue)) {
    throw new Error(`Invalid territory point value: ${pointValue}. Must be 5, 10, or 15.`);
  }
  ...
  const maxLoss = BASE_ATTACK_COST * vpMultiplier;
 ...
 *
 * If defender WINS: BASE_DEFENSE_COST * vpMultiplier * (casualties / totalCasualties)
...
export function calculateDefenderCPLoss(pointValue, casualties, totalCasualties, defenderWon, isFriendlyTerritory) {
  ...
  if (!defenderWon) {
    return isFriendlyTerritory ? DEFENDER_MAX_LOSS.FRIENDLY : DEFENDER_MAX_LOSS.NEUTRAL;
  }
...
export function calculateBattleCPCost({
  ...
  const defender = attacker === 'USA' ?
    (territoryOwner === 'CSA' ? 'CSA' : 'NEUTRAL') :
    (territoryOwner === 'USA' ? 'USA' : 'NEUTRAL');
  
  const totalCasualties = attackerCasualties + defenderCasualties;
  
  // Calculate attacker CP loss (always based on casualties)
...
export function getMaxBattleCPCosts(territoryPointValue, territoryOwner, defender) {
  ...
  const attackerMax = BASE_ATTACK_COST * vpMultiplier;
